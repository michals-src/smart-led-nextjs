...
const SceneryEditor: FC<ISceneryEditor> = props => {
  const { index, previous, next, current, brightness, color, onSave } = props;
  ...
  const currentTime = null === current ? "12:00" : current;
  const nextTime = null === next ? "24:00" : next;

  const startTime = hhmm2time(previousTime);
  const endTime = hhmm2time(nextTime);
  ...
  return (
    <>
      <div className='p-4 mb-4'>
        ...
        <p className='text-sm'>Zmiana ustawień punktu</p>
      </div>
        ...
        <div className='flex flex-row flex-nowrap w-full justify-center items-center relative px-4'>
          ...
          <div className='w-3/12 relative z-10'>
            <Box
              ...
              <p className='text-xs text-zinc-100'>{next}</p>
            </Box>
          </div>
        </div>
      </div>
