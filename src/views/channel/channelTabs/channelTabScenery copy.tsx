import React, { Children, FC, forwardRef, ReactElement, useCallback, useContext, useEffect, useRef, useState } from "react";
import db from "@firebase";
import { update, ref } from "firebase/database";
import { v4 as uuidv4 } from "uuid";

import { ClockIcon, Cog6ToothIcon, FlagIcon, PlayIcon, PlusIcon, TrashIcon, XCircleIcon } from "@heroicons/react/24/solid";
...
export default function ChannelTabScenery({}: IScenery) {
  ...
  return <>{!loading && <div>{data[channelID].length}</div>}</>;

  const [scenerios, setScenerions] = useState<typeof data>(null === data ? [] : data);
  ...
  const SceneryCreate = ({ index, previous, current, brightness, color = "" }) => {
    return (
      <div className='w-1/2 mx-auto px-2 relative'>
        <div
          ...
          style={{
            filter: "blur(40px)",
            background: `linear-gradient(to bottom, rgba(0,0,0,0) 0%, ${"" === color ? "#FFFFFF40" : color} 150%)`,
          }}></div>
        <div
          className='w-1 h-12 absolute top-0 bg-white opacity-[0.4] bottom-0 left-[50%]'
          style={{
            background: `linear-gradient(to bottom, rgba(0,0,0,0) 0%, ${"" === color ? "#FFFFFF40" : color} 100%)`,
  ...
  const SceneryItem = ({ index = 0, type = "normal", label = "", color = "", previous = null, next = null, brightness = 100 }) => {
    return (
      <div>
        ...
        <Box
          className={`px-4 pt-3 ${"end" === type ? "pb-3 " : "pb-1 "} relative z-10`}
          bgGradient={"" === color ? "#FFFFFF15" : color}>
          <div className='flex flex-row flex-nowrap items-center justify-center'>
            <div className='mr-1'>
              {type === "start" && <PlayIcon className='w-5 h-5 text-white' />}
              {type === "normal" && <ClockIcon className='w-5 h-5 text-white' />}
              {type === "end" && <FlagIcon className='w-5 h-5 text-white' />}
            </div>
            <p className={`text-xs text-white ${"end" == type ? "w-full text-center" : "ml-4"}`}>{label}</p>
            {type === "normal" && (
      ...
      <div className='flex flex-col nowrap w-full h-auto'>
        ...
        {scenerios.length - 1 > 0 && (
          <>
            <Box className='p-4 pt-0 '>
              {scenerios.slice(0, scenerios.length - 1).map((sceneryChild: any, idx: number) => {
                return (
                  <>
                    <SceneryItem
                      key={uuidv4()}
                      ...
                      next={idx === scenerios.length - 1 ? "24:00" : scenerios[idx + 1].time}
        ...
        {!loading && (
          <SceneryItem
            ...
            previous={scenerios?.length >= 2 ? scenerios[scenerios.length - 2].time : "00:00"}
            label={scenerios?.length >= 1 ? scenerios[scenerios.length - 1].time : "24:00"}
            type='end'
          />
        )}
      </div>
