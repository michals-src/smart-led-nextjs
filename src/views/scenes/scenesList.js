import { useContext, useState, useEffect, useCallback, forwardRef, isValidElement, cloneElement, useRef } from 'react';
...
import { SceneNew, SceneSettings } from './sheets';
...
const BsRootContent = (props) => {
	/**
	 * @TODO
	 * if we are moving within one component cache navigation element
	 *  to eliminate the possibility of omitting properties
	 *    in case when one of them is invoked from more than one place
	 */
	...
	return (
		<BottomSheet.Content
			...
			onClose={() => {
				//alert('saving ...');
				//props.navigate('screen-2', { foo: 'bar' });
				props.close();
			}}>
			<div>
				Abc
...
const BsScreenTwoContent = (props) => {
	return (
		<BottomSheet.Content onBack={() => props.back()}>
			<div>
				{props.foo}
...
const ScenesList = (props) => {
	...
	return (
		<div className='w-full pt-10 px-3'>
			...
			<BottomSheet
				open={bsOpen['bsNew']}
				onClose={() => handleBsClose('bsNew')}>
				<BottomSheet.View root='true'>
					{/* <SceneNew /> */}
					<SceneSettings
						id={Object.keys(data)[0]}
						childId={data[Object.keys(data)[0]].childrenID}
						name={data[Object.keys(data)[0]].name}
					/>
				</BottomSheet.View>
			</BottomSheet>
			{/* <BottomSheet
				open={bsOpen['bsSettings']}
				onClose={() => handleBsClose('bsSettings')}>
				<BottomSheet.View root='true'>
					<BsRootContent abc='def' />
				</BottomSheet.View>
				<BottomSheet.View as='screen-1'>
					<p className='p-8'>Screen 1</p>{' '}
				</BottomSheet.View>
				<BottomSheet.View as='screen-2'>
					<BsScreenTwoContent />
				</BottomSheet.View>
			</BottomSheet> */}
		</div>
